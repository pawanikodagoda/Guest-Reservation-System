    ...
    <Container className="py-5">
      ...
      <div className="glass-card p-5">
        <div className="table-responsive table-container">
          <Table className="mb-0">
            ...
            <tbody>
              {rooms.map(room => (
                <tr key={room.id} style={{ background: 'rgba(30, 58, 138, 0.3)', color: '#F8FAFC' }}>
                  <td style={{ color: '#F8FAFC', fontWeight: '600' }}>#{room.roomNumber}</td>
                  <td style={{ color: '#F8FAFC' }}>{room.roomType}</td>
                  <td style={{ color: '#F8FAFC', fontWeight: '500' }}>Rs. {room.pricePerNight.toLocaleString()}</td>
                  <td>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered contentClassName="glass-card border-0">
        <Modal.Header closeButton className="border-0 pb-0" closeVariant="white">
          <Modal.Title className="fw-bold">{editingRoom ? 'Update Suite' : 'Add New Suite'}</Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleSubmit}>
          <Modal.Body className="p-4">
            <Row className="g-3">
              ...
              <Col md={6}>
                <Form.Group>
                  <Form.Label className="text-muted small fw-bold text-uppercase">Type</Form.Label>
                  <Form.Select name="roomType" className="form-control-lg" value={formData.roomType} onChange={handleChange} required>
              ...
              <Col md={6}>
                <Form.Group>
                  <Form.Label className="text-muted small fw-bold text-uppercase">Rate / Night (Rs.)</Form.Label>
                  <Form.Control
                    type="number"
                    name="pricePerNight"
                    className="form-control-lg"
              ...
              <Col md={12}>
                <Form.Group>
                  <Form.Label className="text-muted small fw-bold text-uppercase">Status</Form.Label>
                  <Form.Select name="status" className="form-control-lg" value={formData.status} onChange={handleChange} required>
          ...
          <Modal.Footer className="border-0 pt-0 p-4">
            ...
            <Button variant="primary" type="submit" className="px-4 py-2 d-flex align-items-center gap-2">
              {editingRoom ? 'Save Changes' : 'Add Suite'}
            </Button>
          </Modal.Footer>
        </Form>
      </Modal>
